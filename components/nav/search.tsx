...
const Search = ({className, style}: Props) => {
  return (
      ...
      <span className="sr-only">Search</span>
        ...
        <input className="placeholder:italic placeholder:text-slate-400 text-black bg-white w-[80%] sm:w-4/6 md:w-5/6 border border-slate-300 rounded-2xl sm:rounded-xl py-1 pl-9 pr-3 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm" style={style} placeholder='Artists, songs or podcasts' type="text" name="search" />
      </label>
  )
}
