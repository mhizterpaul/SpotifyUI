...
const Sidebar = (params: { isOpen?: boolean }) => {
    ...
    return (<aside className={/*my-auto */ [`bg-black sidebar hidden sm:flex flex-col h-[90vh] min-h-[560px] items-center max-h-[914px] justify-around
        ...
        <ul className='self-start'>
            ...
            <li onClick={() => dispatch(pushRef('/library?playlists'))}><svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24"><path fill="currentColor" d="M7 3h2v18H7zM4 3h2v18H4zm6 0h2v18h-2zm9.062 17.792l-6.223-16.89l1.877-.692l6.223 16.89z" /></svg><div className='flex flex-col justify-start items-center font-bold capitalize'> Your library {Playlist.length || ownPlaylist.length ? <MediaCount type={'playlist'} />: null}</div></li>
        </ul>
        <ul className='self-start'>
            ...
            <li onClick={()=>dispatch(pushRef('/episodes'))} className=''>
                ...
                <div className='flex flex-col justify-start items-center font-bold capitalize'> your episodes {Episodes.length ? <MediaCount type={'episode'} />: null}</div> </li>
        </ul>
