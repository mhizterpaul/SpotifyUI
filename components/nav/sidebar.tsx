...
const Sidebar = ({ Playlist, Tracks, Episodes, isOpen }: V & { isOpen?: boolean }) => {
    ...
    return (<aside className={/*my-auto */[`bg-black sidebar hidden sm:flex flex-col h-[90vh] min-h-[560px] items-center max-h-[914px] justify-around
        ...
        <ul className='self-start'>
            ...
            <li onClick={() => dispatch(pushRef('/library'))}><svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24"><path fill="currentColor" d="M7 3h2v18H7zM4 3h2v18H4zm6 0h2v18h-2zm9.062 17.792l-6.223-16.89l1.877-.692l6.223 16.89z" /></svg><div className='flex flex-col justify-start items-center capitalize'> Your library {Object.keys(Playlist).length ? <MediaCount type={'playlist'} /> : null}</div></li>
        </ul>
        <ul className='self-start'>
            ...
            <li onClick={() => dispatch(pushRef('/playlist/songs'))}>
                ...
                <div className='flex flex-col justify-start items-center capitalize'> liked songs {Object.keys(Tracks).length ? <MediaCount type={'song'} /> : null}</div></li>
            <li onClick={() => dispatch(pushRef('/episodes'))} className=''>
                ...
                <div className='flex flex-col justify-start items-center capitalize'> your episodes {Object.keys(Episodes).length ? <MediaCount type={'episode'} /> : null}</div> </li>
        </ul>
