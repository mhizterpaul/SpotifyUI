...
const Nav = ({search, isMobile, route, next, prev} : Props) => {
    ...
    return (
        <>
            <nav className={`flex relative sm:col-start-2 ${isOpen? 'col-start-2' : 'col-start-1'} col-end-4 w-full flex-row nav items-center justify-between py-2`}>
                ...
                {!isOpen && <svg xmlns="http://www.w3.org/2000/svg" width="60" height="60" className='sm:hidden absolute right-0 ' onClick={() => {setIsOpen(true); dispatch(setOpen(true))}} viewBox="0 0 72 72"><path fill="none" stroke="#000" strokeLinecap="round" strokeLinejoin="round" strokeMiterlimit="10" strokeWidth="2" d="M16 26h40M16 36h40M16 46h40" /></svg>}
                {
                    ...
                    //with the other box
                   isOpen && <div className='sm:hidden w-[3.75rem] h-[60px] overflow-hidden flex flex-col items-center justify-center text-black absolute right-0' onClick={() => {setIsOpen(false); dispatch(setOpen(false))}}>
