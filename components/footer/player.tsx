...
const Player: React.FC<{}> = () => {
    ...
    useMemo(() => currentPlaylist && setShuffledQueue(state => ({ ...state, queue: currentPlaylist.tracks || currentPlaylist.items || currentPlaylist.episodes })), [currentPlaylist])
    ...
    const handlePrevious = () => {
        ...
        if (shuffledQueue.queue[shuffledQueue.queue.indexOf(nowPlaying)] === 0 && repeat) {
        ...
        if (shuffledQueue.queue[shuffledQueue.queue.indexOf(nowPlaying)] === 0 && !repeat) return setProp('nowPlaying', null);
        setProp('nowPlaying', shuffledQueue.queue[shuffledQueue.queue.indexOf(nowPlaying) - 1]);
    };

    const handleNext = () => {
        if (!shuffledQueue.queue || !nowPlaying) return;
        if (shuffledQueue.queue[shuffledQueue.queue.indexOf(nowPlaying)] === shuffledQueue.queue.length - 1 && repeat) {
            if (shuffledQueue.shuffle) setShuffledQueue(state => ({ ...state, queue: shuffle(state.queue) }))
            setProp('nowPlaying', shuffledQueue.queue[0]);
            return
        }
        if (shuffledQueue.queue[shuffledQueue.queue.indexOf(nowPlaying)] === shuffledQueue.queue.length - 1 && !repeat) return setProp('nowPlaying', null);
        setProp('nowPlaying', shuffledQueue.queue[shuffledQueue.queue.indexOf(nowPlaying) + 1]);
            ...
            <div className={`w-[50vw] min-w-[150px] max-w-[350px]`}>
                ...
                <section className='player flex flex-col items-center '>
                    <div className='flex w-4/6 flex-nowrap mb-2 text-[#B3B3B3] justify-between [&_svg:hover]:scale-105 [&_svg:hover]:text-white items-center'>
                        <ActiveDot>
                            <RxShuffle className={' hover:scale-105 '} onClick={() => setShuffledQueue(state => ({ shuffle: !state.shuffle, queue: !state.shuffle ? shuffle(state.queue) : currentPlaylist?.tracks || currentPlaylist?.items || currentPlaylist?.episodes }))} />
                        </ActiveDot>
                        <MdSkipPrevious className={' hover:text-white' + style.twinkle} onClick={handlePrevious} />
                        {isPlaying ? <BsPauseCircle className={'play text-white ' + style.button} onClick={() => { setIsPlaying(false); togglePlay() }} /> : <BsPlayCircle className={'play text-white ' + style.button} onClick={() => { setIsPlaying(true); togglePlay() }} />}
                    ...
                    <div className='w-full flex justify-center gap-x-2 items-center'><span id="current-time" className="pr-2 text-xs">{(nowPlaying ? Math.trunc(currentTime / 60) : '-') + ' : ' + (nowPlaying ? Math.round(currentTime % 60) : '- -')}</span>
                        <input type='range'
                            ...
                            defaultValue={nowPlaying && audioRef.current ? currentTime * (100 / audioRef.current.duration) : 0}
