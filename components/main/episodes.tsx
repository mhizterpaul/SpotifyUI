...
const Episodes = () => {
    ...
    if (!access_token) return <div className='text-center my-auto text-2xl'><RiErrorWarningLine className='text-5xl block mx-auto mb-4' />something went wrong </div>;

    if (!episodeData && pathname.includes('/episode/')) return <div className='text-center my-auto text-xl font-extrabold'><div className='text-2xl'><RiErrorWarningLine /></div>Couldn't find that episode <br /> <span className='text-sm font-semibold'>search for something else?</span> </div>;
    ...
    if (pathname === '/episodes' && Object.keys(Episodes).length) return (
        <section>
            <h2 className={'p-2 pt-4 flex bg-[#045141] gap-x-2 max-h-[375px] h-[50%]' + theme !== 'bg-[' ? theme : ''}>
                    ...
                    {Object.values(Episodes).map((episode: Episode, index: number) => (() => {
                        ...
                        return <div onMouseEnter={() => setHover(true)}>
                            ...
                            <div className='relative'>
                                <BsPlayCircle /> {episode.release_date} &bull; {episode.duration_ms / 60 + ' min'} {episode.duration_ms % 60 + ' sec'} <div className={'relative '}><IoShareOutline className={tooltip + (hover ? 'inline-block' : 'hidden') + ' hover:before:content-["Share"] '} /></div> <div className={'relative '}><BiSolidCheckCircle className={tooltip + " hover:before:content-['Remove from Your Library'] "} onClick={() => removeMedia('Episodes', episode.id)} /></div> <div className={'relative '}><SlOptions className={tooltip + (hover ? 'inline-block' : 'hidden') + optionsContent} onClick={() => setEpisodeSubMenu(true)} /></div>
    ...
    return (
        <section>
            <h2 className=' flex items-center bg-[7A2D85] justify-around'>
                {bgColor ? <Image src={episodeData.image} width={100} height={100} alt={episodeData.name} /> : <BgColorDetector imageUrl={episodeData.image} callBack={(hexCode) => setBgColor(hexCode)} />}
            ...
            <section className='bg-gradient-to-b from-[491B4F] to-[121212]'>
                <h2 className='flex flex-col h-16'>
                    ...
                    <div className='flex gap-x-4 text-[1ED760] justify-start items-center relative'><BsPlayCircle className=' hover:scale-110' /><LuPlusCircle className={'hover:scale-110 '} /><BiSolidCheckCircle aria-hidden className={`hidden`} /><SlOptions className={'text-[A7A7A7] hover:before:absolute hover:before:bottom-[110%] hover:before:right-[50%] hover:before:p-2 hover:before:bg-zinc-900 hover:before:shadow-md ' + hover} /></div>
                </h2>
                <p>
                    ...
                    <button type='submit' aria-disabled onClick={() => { }} className='rounded-2xl h-6 p-2 text-base font-bold capitalize border-2 border-[smokewhite] hover:scale-110'>
                        see all episodes
                    </button>
                </p>
            </section>
