...
const Library = () => {
  ...
  if (!(Playlist.length + ownPlaylist.length + Tracks.length + Episodes.length)) return (
  <section className='w-full h-full flex flex-col items-center justify-center gap-y-8'>
    <div className='text-sm'><h3 className='text-base font-bold'>Create your first playlist</h3>
    it's easy we'll help you
    <button onClick={() => setCloseForm(false)} className='round-xl bg-white text-black p-6 font-bold hover:scale-130'>Create playlist</button>
    </div>
    <div className='text=sm'>
      <h3 className='text-base font-bold'>let's find you some podcasts to follow</h3>
      We'll keep you updated on new Episodes
      <button onClick={() => (
        <SeeAll title={'Shows'} callBack={(index)=> getSeveralShows(access_token, index, '6')}/>
      )} className='round-xl bg-white text-black p-6 font-bold hover:scale-130'>Browse podcasts</button> 
    </div>
  </section>
  )
  ...
  return <>
    {
      (Playlist.length + ownPlaylist.length && (window.location.href.split('library').length === 1 || searchParams.get('new') === 'true')) && <section>
